...
function getProfilePath(storeId) {
  const profilePath = path.join(os.tmpdir(), `trail-profile-${storeId}`);
...
async function handleLogin(page, credentials, reportType) {
  try {
    ...
    const fs = require('fs');
    const path = require('path');
    const errorDir = path.join('/tmp', 'trail-errors');
...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    console.error('Error in trail-session handler:', error);
    return {
      statusCode: 500,
      body: JSON.stringify({ 
        success: false, 
