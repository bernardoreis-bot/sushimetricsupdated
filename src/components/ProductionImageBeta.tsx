...
import { Camera, Upload, CheckCircle, AlertCircle, Loader, Info, TrendingUp, XCircle, RefreshCw, AlertTriangle, Key, Settings, Download, Save } from 'lucide-react';
...
export default function ProductionImageBeta() {
  ...
  const processImage = async () => {
    ...
    try {
      ...
      for (const req of summary.requirements) {
        await supabase.from('production_requirements').insert({
          site_id: selectedSite,
          week_ending_date: latestWeek.formattedDate,
    ...
    <div className="p-8">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl shadow-lg p-8 mb-8 text-white">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-blue-100 mt-1">Complete production planning with OpenAI and PowerBI data</p>
          </div>
        </div>
      ...
      {selectedSite && (
        <>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Step 1: Sales Data (Last 3 Weeks)</h2>
              ...
              {weeks.map((week) => {
                ...
                return (
                  <div key={week.weekNumber} className={`border-2 rounded-lg p-4 ${
                    ...
                    />
                    {hasFile && (
                      <p className="text-xs text-blue-600 mt-2">✓ New file ready to upload</p>
                    )}
                  </div>
                );
              })}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Step 2: Production Plan Image</h2>

            {storedProductionImage && (
              <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-blue-900">✓ Previous Image Saved</p>
                    ...
                    <p className="text-xs text-blue-600 mt-1">Upload a new image to replace or view previous results below</p>
                  </div>
                  {showResults && (
                    <button
                      onClick={() => {
                        const element = document.getElementById('production-results');
                        element?.scrollIntoView({ behavior: 'smooth', block: 'start' });
                      }}
                      className="ml-4 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg transition-colors"
                    >
                      View Results
                    </button>
                  )}
            ...
            >
              {isProcessing ? (
                ...
                <>
                  ...
                  Process Image & Calculate Requirements
                </>
              )}
            </button>
          ...
          {showResults && productionSummary && (
            <div id="production-results" className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h2 className="text-2xl font-bold text-gray-800 mb-6">Production Requirements</h2>

              <div className="grid grid-cols-4 gap-4 mb-6">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-sm font-semibold text-gray-700 mb-1">Total Items</p>
                  <p className="text-3xl font-bold text-blue-600">{productionSummary.totalItems}</p>
                </div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {productionSummary.requirements.map((req, index) => (
                      <tr key={index} className={req.isReduced ? 'bg-purple-50' : req.matchStatus === 'needs_review' ? 'bg-amber-50' : ''}>
                        ...
                        <td className="px-4 py-3 text-sm text-right font-semibold text-blue-600">{req.suggestedProduction}</td>
              ...
              <div className="mt-6 space-y-3">
                <div className="p-4 bg-blue-50 border-l-4 border-blue-500 rounded-lg">
                  <p className="text-sm text-blue-800">
                    <strong>Note:</strong> Unit prices are reduced to 85% of sales price for cost calculation.
                    Average weekly sales and production calculated from {productionSummary.requirements[0]?.weeks || 0} week(s) of data.
                  </p>
                </div>

                <div className="p-4 bg-green-50 border-l-4 border-green-500 rounded-lg">
                  <h4 className="font-semibold text-gray-800 mb-3">Buffer Percentages & Reduced Item Distribution:</h4>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs mb-3">
                    <div className="bg-white p-2 rounded border border-green-200">
                      <div className="font-semibold text-gray-700">£2.51-£3.49 → £2.50</div>
                      <div className="text-green-700">5% buffer</div>
                    </div>
                    <div className="bg-white p-2 rounded border border-green-200">
                      <div className="font-semibold text-gray-700">£3.60-£4.49 → £3.50</div>
                      <div className="text-green-700">7% buffer</div>
                    </div>
                    <div className="bg-white p-2 rounded border border-green-200">
                      <div className="font-semibold text-gray-700">£4.70-£6.90 → £4.50</div>
                      <div className="text-green-700">8% buffer</div>
                    </div>
                    <div className="bg-white p-2 rounded border border-green-200">
                      <div className="font-semibold text-gray-700">£7.00-£9.99 → £6.00</div>
                      <div className="text-green-700">10% buffer</div>
                    </div>
                    <div className="bg-white p-2 rounded border border-green-200">
                      <div className="font-semibold text-gray-700">£10.00-£14.00 → £7.50</div>
                      <div className="text-green-700">12% buffer</div>
                    </div>
                    <div className="bg-white p-2 rounded border border-green-200">
                      <div className="font-semibold text-gray-700">£16+ → £15.00</div>
                      <div className="text-green-700">15% buffer</div>
                    </div>
                  </div>
                  <div className="bg-purple-50 p-3 rounded border border-purple-200">
                    <p className="text-xs text-gray-700 mb-1">
                      <strong>Reduced Item Distribution:</strong> Total reduced items in each price range are divided equally among all non-reduced items in that same price range, then added to their suggested weekly production.
                    </p>
                    <p className="text-xs text-purple-700 mt-1">
                      <strong>Example:</strong> If 10 items are reduced in the £4.70-£6.90 range, and there are 2 items (Edamame, Inari) in that range, each gets +5 added to their weekly recommendation.
                    </p>
                  </div>
                </div>
              </div>
      ...
      {diagnosticLog.length > 0 && (
        <div className="mt-6 bg-gray-900 rounded-xl p-4">
          <p className="text-xs font-semibold text-gray-300 mb-2">Activity Log:</p>
          <div className="bg-black rounded p-3 max-h-60 overflow-y-auto">
