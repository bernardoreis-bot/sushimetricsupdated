...
// netlify/functions/dropbox-config.ts
var dropbox_config_exports = {};
__export(dropbox_config_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(dropbox_config_exports);
...
var SETTING_KEY = "dropbox_credentials";
...
async function fetchConfig() {
  const { data } = await supabaseAdmin.from("app_settings").select("id, setting_value, updated_at").eq("setting_key", SETTING_KEY).maybeSingle();
  if (!data?.setting_value) {
    return { configured: false, appKey: null, updatedAt: data?.updated_at ?? null };
  }
  try {
...
async function requireAdmin(token) {
  ...
  const { data: profile, error: profileError } = await supabaseAdmin.from("user_profiles").select("user_roles!inner(name)").eq("id", user.id).maybeSingle();
...
var handler = async (event) => {
  ...
  if (event.httpMethod === "POST") {
    try {
      ...
      const { data } = await supabaseAdmin.from("app_settings").select("id").eq("setting_key", SETTING_KEY).maybeSingle();
      if (data?.id) {
...
//# sourceMappingURL=dropbox-config.js.map
