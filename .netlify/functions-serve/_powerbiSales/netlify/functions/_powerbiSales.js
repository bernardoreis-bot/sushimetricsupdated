...
// netlify/functions/_powerbiSales.ts
var powerbiSales_exports = {};
__export(powerbiSales_exports, {
  createSalesTransactions: () => createSalesTransactions,
  getLastSundayISO: () => getLastSundayISO,
  getSalesCategoryId: () => getSalesCategoryId,
  loadConfig: () => loadConfig,
  runExtractCurrentView: () => runExtractCurrentView,
  runScheduledBatch: () => runScheduledBatch,
  saveAudit: () => saveAudit,
  supabaseAdmin: () => supabaseAdmin
});
module.exports = __toCommonJS(powerbiSales_exports);
...
var POWERBI_EMBED_URL = "https://app.powerbi.com/reportEmbed?reportId=c016912d-8d76-4829-85bb-e2f4056dd807&appId=eb0beee5-b009-4dee-816d-21adae41cf84&autoAuth=true&ctid=d1fd9353-f65b-4e79-a56a-70b4591ad484&actionBarEnabled=true&reportCopilotInEmbed=true";
...
async function loadConfig() {
  const { data } = await supabaseAdmin.from("app_settings").select("setting_value").eq("setting_key", "powerbi_sales_config").maybeSingle();
  try {
...
async function saveAudit(entry) {
  const { data } = await supabaseAdmin.from("app_settings").select("id, setting_value").eq("setting_key", "powerbi_sales_audit").maybeSingle();
  let list = [];
  try {
    list = data?.setting_value ? JSON.parse(data.setting_value) : [];
  } catch {
  }
  list.unshift({ time: (/* @__PURE__ */ new Date()).toISOString(), ...entry });
  const payload = { setting_key: "powerbi_sales_audit", setting_value: JSON.stringify(list.slice(0, 200)), updated_at: (/* @__PURE__ */ new Date()).toISOString() };
  if (data?.id) {
...
async function getSalesCategoryId() {
  const { data } = await supabaseAdmin.from("transaction_categories").select("id, code").eq("code", "SALES").maybeSingle();
  return data?.id || null;
}
async function screenshotReport(url) {
...
async function extractCurrentViewFromScreenshot(base64Png) {
...
async function createSalesTransactions(amountBySiteId, sundayISO, source) {
...
async function runExtractCurrentView(source) {
  const cfg = await loadConfig();
  const siteMap = cfg.site_map || {};
  ...
  const match = Object.keys(siteMap).find((k) => k.toLowerCase() === (site_name || "").toLowerCase()) || Object.keys(siteMap).find((k) => (site_name || "").toLowerCase().includes(k.toLowerCase())) || Object.keys(siteMap).find((k) => k.toLowerCase().includes((site_name || "").toLowerCase()));
  const siteId = match ? siteMap[match] : void 0;
  const bySiteId = {};
...
async function siteIdByName(name) {
  const { data } = await supabaseAdmin.from("sites").select("id, name, is_active").eq("is_active", true);
  const match = (data || []).find((s) => (s.name || "").toLowerCase() === name.toLowerCase());
  return match?.id || null;
}
async function hasWeeklySalesFor(site_id, sundayISO) {
  const { data } = await supabaseAdmin.from("transactions").select("id").eq("site_id", site_id).eq("transaction_date", sundayISO).limit(1);
  return !!(data && data.length > 0);
}
async function runScheduledBatch() {
  const sundayISO = getLastSundayISO();
  const { data: autoCfgRow } = await supabaseAdmin.from("app_settings").select("setting_value").eq("setting_key", "powerbi_sales_auto").maybeSingle();
  let autoCfg = {};
  try {
    autoCfg = autoCfgRow?.setting_value ? JSON.parse(autoCfgRow.setting_value) : {};
  } catch {
  }
  const embedUrls = autoCfg.embed_urls || {};
  const sites = ["Allerton Road", "Sefton Park", "Old Swan"];
  const results = [];
  for (const siteName of sites) {
    const site_id = await siteIdByName(siteName);
    if (!site_id) {
      await saveAudit({ type: "batch_skip", reason: "site_not_found", siteName });
      continue;
    }
    const already = await hasWeeklySalesFor(site_id, sundayISO);
    if (already) {
      await saveAudit({ type: "batch_skip", reason: "already_created", siteName, sundayISO });
      continue;
    }
...
0 && (module.exports = {
  ...
  getSalesCategoryId,
  loadConfig,
  runExtractCurrentView,
  runScheduledBatch,
  saveAudit,
  supabaseAdmin
});
//# sourceMappingURL=_powerbiSales.js.map
