...
// netlify/functions/dropbox-auth.ts
var dropbox_auth_exports = {};
__export(dropbox_auth_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(dropbox_auth_exports);
var import_supabase_js = __toESM(require_main5(), 1);
var DROPBOX_TOKEN_URL = "https://api.dropbox.com/oauth2/token";
...
var supabaseAdmin = SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY ? (0, import_supabase_js.createClient)(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
  auth: {
    autoRefreshToken: false,
    persistSession: false
  }
}) : null;
var SETTINGS_KEY = "dropbox_credentials";
var getStoredCredentials = async () => {
  if (!supabaseAdmin) {
    return null;
  }
  const { data, error } = await supabaseAdmin.from("app_settings").select("setting_value").eq("setting_key", SETTINGS_KEY).maybeSingle();
...
//# sourceMappingURL=dropbox-auth.js.map
